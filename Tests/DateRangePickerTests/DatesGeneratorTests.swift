...
final class DatesGeneratorTests: XCTestCase {
	...
	func testGenerateYears_CustomValues() {
		...
		let maximumYear = 2021
		let minimumDate = Date.date(from: "\(minimumYear)-05-09")
		let maximumDate = Date.date(from: "\(maximumYear)-11-11")
		let yearDates = [minimumDate] + (minimumYear+1...maximumYear)
